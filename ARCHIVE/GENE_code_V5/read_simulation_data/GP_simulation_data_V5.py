...
import os
import pandas as pd
from GP_file_functions_V4 import suffix_from_filename, string_to_list
from find_buried_filetypes import find_buried_filetype_files
...
os.chdir(os.path.join(cwd, 'criteria_checker'))
from filetype_key_lists_V4 import load_files_from_criteria
...
def filepath_to_simulation_dict_list(filepath_list, criteria_list=[], load_spec='all', debug:bool=False):
    ...
    criteria_dict_list = criteria_list_to_dict_list(criteria_list)
    ...
    if len(parameter_filepath_list) == 0:
        ...
        for parameter_filepath in parameter_filepath_list:
            ...
            simulation_dict = simulation_filepath_to_dict(simulation_directory, suffix, load_spec, criteria_dict_list)
...
def simulation_filepath_to_dict(simulation_filepath:str, suffix:str, load_spec:list, criteria_dict_list:list):
    """
    ...
    parameter_filepath = load_simulation_filepath(simulation_dict, 'parameters')
    parameter_dict = parameters_filepath_to_dict(parameter_filepath)
    simulation_dict['parameters_dict'] = parameter_dict

    load_files = load_files_from_criteria(criteria_dict_list)
    # print(load_files)
    ...
    if 'nrg' in load_files:
        ...
        nrg_dict = nrg_filepath_to_dict(nrg_filepath, time_slice, load_spec, nrg_quantities)
