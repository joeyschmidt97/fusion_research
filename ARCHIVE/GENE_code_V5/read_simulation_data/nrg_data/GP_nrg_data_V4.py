...
# TODO: UPDATE to fix nrg dict and 

import numpy as np

import os
os.chdir("..")
from GP_file_functions_V4 import file_checks, FileError, switch_suffix_file
from criteria_checker.filetype_key_lists_V4 import nrg_key_list

# os.chdir("./parameters_data")
from parameters_data.GP_parameters_data_V4 import species_list, parameters_filepath_to_dict
...
def create_nrg_coordinate_list(nrg_filepath:str, named_spec_row = 'all', named_nrg_col = 'all'):
    ...
    parameters_filepath = switch_suffix_file(nrg_filepath, 'parameters')
    spec_tuple = species_list(parameters_filepath) #get species as [('i', 1), ('e', 2), ('c', 3)] or some variant

    #get nrg_quantity list as [('n_mag', 0), ('u_par_mag', 1), ('T_par_mag', 2), ...] or some variant
    MOD_nrg_key_list = nrg_key_list[:]
    MOD_nrg_key_list.remove('time') #remove time from nrg columns
    nrg_quantity_tuple = tuple((nrg_key, col) for col, nrg_key in enumerate(MOD_nrg_key_list))
