...
def MTM_color_plot(filepath, color_bar = 'local', show_z_vals = False):
    ...
    scatter = ax.scatter(beta_list, temp_list, coll_list, edgecolor='black')
        ...
        # Scale the values to determine the size of the points
    s = 50 + KBL_limit * 200  # Adjust the scaling factor (200) to control the size range

    # Plot the points with size representing the 4th dimension (values) and an outline for values close to 1
    c = KBL_limit
    for i in range(len(KBL_limit)):
        if KBL_limit[i] >= 0.9:  # You can adjust the threshold value (0.9) for the outline
            ax.scatter(beta_list[i], temp_list[i], coll_list[i], c='r', s=s[i], edgecolors='k', linewidth=1.5)
        else:
            ax.scatter(beta_list[i], temp_list[i], coll_list[i], c=c[i], s=s[i], cmap='viridis')


    # Add color bar (optional)
    cbar = plt.colorbar(ax.scatter(beta_list, temp_list, coll_list, c=c, cmap='viridis'))
    cbar.set_label('4th Dimension Value')
