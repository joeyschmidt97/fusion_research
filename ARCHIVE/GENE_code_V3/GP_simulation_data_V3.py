...
def species_value_from_simulation(simulation_dict:dict, variable_key:str, species_name:str) -> str:
    """
    Retrieves the variable key associated with a specified species from a simulation dictionary.

    Parameters:
    - simulation_dict (dict): Dictionary containing simulation parameters and metadata.
    - variable_key (str): The key prefix for the desired variable.
    - species_name (str): The name of the target species.

    Returns:
    - str: The complete variable key associated with the given species, or an empty string if not found.
    """
    
    variable_species_key = ''  # Placeholder for the desired key
    ...
    for i in range(parameters_dict['n_spec']):
        spec_num = i + 1  # Adjust for 1-based indexing
        spec_name = 'name' + str(spec_num)  # Construct the species name key (e.g., "name1", "name2", ...)
        ...
        if current_species_name == species_name.strip("'"):
            variable_species_key = variable_key + str(spec_num)  # Construct the desired key
            found = True
            break  # Exit the loop since the target species has been found

    # If the desired key wasn't found, display an error message and print available species labels
    if not found:
        print(f'No such key {variable_key} could be found. Check you have the right variable name and species label used in the GENE parameters file.')
...
#------------------------------------------------------------------------------------------------
# Functions to get all simulation files from a directory and load filepath for given filetype----
#------------------------------------------------------------------------------------------------
