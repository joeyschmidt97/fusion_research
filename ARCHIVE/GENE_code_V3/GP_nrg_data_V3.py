...
from GP_parameter_data_V3 import species_name_to_number
...
def nrg_filepath_to_dict_V2(nrg_filepath: str, parameters_dict:dict, spec_type:str=None, time:str=None, debug: bool = False):
    """
    Converts a 'nrg' file at a given filepath to a nrg dictionary. There are controls to check which species
    you want info for or what time you want to sample (None -> all time, )
    Returns the nrg dictionary.
    """
    ...
    if (spec_type != None):
        target_species_number = species_name_to_number(parameters_dict, species_name=spec_type)
    else:
        target_species_number = None
